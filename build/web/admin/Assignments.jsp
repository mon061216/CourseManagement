    ...
    <body class="animated-bg">
        <div class="container mt-4 mb-5">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 class="fw-bold text-dark mb-1">Student Scores</h2>
                    <p class="text-muted small fw-bold mb-0">Class: <span class="text-primary">${CLASS_ID}</span> | Student: <span class="text-info">${STUDENT_ID}</span></p>
                </div>
                <form action="MainController" method="post" class="m-0">
                    <input type="hidden" name="classID" value="${CLASS_ID}">
                    <button class="btn btn-secondary btn-sm shadow-sm rounded-pill px-3" type="submit" name="action" value="ViewStudents">
            ...
            <c:if test="${empty GRADE_LIST}">
                <div class="alert alert-warning border-0 shadow-sm rounded-3">No scores found for this student.</div>
            </c:if>
                            ...
                            <tbody>
                                <c:forEach var="s" items="${GRADE_LIST}">
                                    <tr>
                                        ...
                                        <td><span class="badge bg-primary bg-opacity-75 rounded-pill px-3">${s.gradeScore}</span></td>
                                        <td>${s.gradeDate}</td>
                                        <td class="text-center">
                                            <form action="MainController" method="POST" class="m-0">
                                                ...
                                                <button name="action" value="ViewSubmission" class="btn btn-premium btn-sm rounded-pill px-3 shadow-sm">
                                                    View Details
                                                </button>
                                            </form>
                                        </td>
